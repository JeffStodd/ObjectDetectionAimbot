...
def main(_argv):
    ...
    while True:
        prev = []
        while win32api.GetAsyncKeyState(win32con.VK_MENU):#win32con.VK_XBUTTON1):#VK_MENU): #alt is being held
            #ctypes.windll.user32.BlockInput(True) #need admin access
            active = True
            t1 = time.time()
            ...
            if scores[0][0] > 0.5 and classes[0][0] == 0.0:
                ...
                if(not np.any(prev)): #prev tuple is empty
                    #ctypes.windll.user32.BlockInput(True) #needs admin access
                    prevTime = t1 + 0
                    '''
                    ...
                    y = int((detection[1] * 1.07)) - int(windowSize/2)
                    #y = int((detection[1] + detection[3])/2 - 208)

                    currTime = time.time()
                    vx = ((x - prev[0])/(currTime - prevTime)) #time between now and last 2 detections
                    vy = ((y - prev[1])/(currTime - prevTime))
                    ...
                    move.mi.dx = x + (int)(vx*(currTime-t1 + timeToShoot)) #time between last detection and moving the mouse + approximate time to execute SendInput
                    move.mi.dy = y + (int)(vy*(currTime-t1 + timeToShoot)) 
                    ...
                    timeToShoot = time.time() - currTime
                    ...
                    #print((int(move.mi.dx + windowSize/2)-1, int(move.mi.dy + windowSize/2)-1), (int(move.mi.dx + windowSize/2)+1, int(move.mi.dy + windowSize/2)+1))
                    #source = cv2.rectangle(source, (int(move.mi.dx + windowSize/2)-1, int(move.mi.dy + windowSize/2)-1), (int(move.mi.dx + windowSize/2)+1, int(move.mi.dy + windowSize/2)+1), (0,255,0), 5)
        ...
        if active and win32api.GetAsyncKeyState(win32con.VK_LBUTTON):
            res = ctypes.windll.user32.SendInput(1, ctypes.pointer(up), ctypes.sizeof(up))
            #ctypes.windll.user32.BlockInput(False) #need admin access
            active = False
